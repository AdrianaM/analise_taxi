...
df_redq_coord = read.csv('redq_coord.csv')
...
## Mapa das corridas que deixaram o passageiro no quadrante
...
leaflet(data = df_dropoff_dentroquadr_sample) %>% 
  ...
  addCircleMarkers(df_pickup_dentroquadr_sample$pickup_longitude, df_pickup_dentroquadr_sample$pickup_latitude, radius = 1, color = "blue", fillOpacity = 0.3) %>% 
      addCircleMarkers(df_ny_dentroquadr$longitude, df_ny_dentroquadr$latitude, radius = 10, color = 'green', fillOpacity = 0.3)


```

### Analises Básicas sobre 

```{r}
glimpse(df_pickup_dentroquadr_sample)

#head(df_pickup_dentroquadr_sample)
```

## As viagens que pegam os passageiros no quadrante tem um desivo padrão menor, pois provavelmente as pessoas estão pegando taxi na Penn Station (local bem no centro do quadrante). 
...
linearMod <- lm(dropoff_lat ~ dropoff_lon, data=df_dropoff_dentroquadr_sample) 
print(linearMod)

linearMod <- lm(dropoff_lat ~ dropoff_lon, data=df_pickup_dentroquadr_sample) 
print(linearMod)

```
```{r}
dropoff.lm = lm(dropoff_lat ~ dropoff_lon, data=df_dropoff_dentroquadr_sample)
summary(dropoff.lm)$r.squared

pickup.lm = lm(pickup_lat ~ pickup_lon, data=df_pickup_dentroquadr_sample)
summary(pickup.lm)$r.squared
```
## plot dos pontos de interesse do quadrante
```{r}


leaflet(data = df_ny_dentroquadr) %>%
    addProviderTiles("Esri.NatGeoWorldMap") %>%
    addCircleMarkers(df_ny_dentroquadr$longitude, df_ny_dentroquadr$latitude, radius = .5, color = 'green', fillOpacity = 0.3)
