...
```{r}
df_ny_dentroquadr %>% 
  select('city','facname','facgroup')

df_nysubway_dentroquadr %>% 
  select('Station.Name','Station.Latitude','Station.Longitude')
```
...
linearMod <- lm(pickup_lat ~ pickup_lon, data=df_pickup_dentroquadr_sample) 
print(linearMod)
