...
## Prepara data e hora da paprtida (acho que nao precisa da chegada)
...
## Transforma as vari�veis de tempo em senoides
```{r}
...
```
```{r}
x <- kmeans_data[, c("trip_duration")]
y <- x[!x %in% boxplot.stats(x)$out]
d <- boxplot(y, las=1, xlab="trip_duration")
...
```{r}
kmeans_no_outliers
boxplot(kmeans_no_outliers[, c("trip_duration")], las=1, xlab="trip_duration")
boxplot(kmeans_no_outliers[, c("dist_euclidiana")], las=1, xlab=c("dist_euclidiana"))
```
...
# de mes na coluna month e de dia da semana na coluna 
df %>%
...
```{r}
X <- df[c('passenger_count', 'dist_manhattan', 'sen_hour', 'sen_week', 'sen_month',
          'bairro_chegada_1','bairro_chegada_2', 'bairro_chegada_3', 'bairro_chegada_4',
         'bairro_chegada_5', 'bairro_chegada_6', 'bairro_chegada_7', 'bairro_chegada_8', 
         'bairro_chegada_9')]
